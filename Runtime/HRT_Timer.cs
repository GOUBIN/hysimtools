...
public class HRT_Timer {
    ...
    private List<HR_TimeTask> tmpTimeLst = new List<HR_TimeTask>();
    private List<HR_TimeTask> taskTimeLst = new List<HR_TimeTask>();
    private List<int> tmpDelTimeLst = new List<int>();

    private int frameCounter;

    private static readonly string lockFrame = "lockFrame";
    private List<HR_FrameTask> tmpFrameLst = new List<HR_FrameTask>();
    private List<HR_FrameTask> taskFrameLst = new List<HR_FrameTask>();
    private List<int> tmpDelFrameLst = new List<int>();

    public HRT_Timer(int interval = 0) {
        tidLst.Clear();
        recTidLst.Clear();
            ...
            lock (lockTime) {
                for (int i = 0; i < tmpDelTimeLst.Count; i++) {
                    ...
                    for (int j = 0; j < taskTimeLst.Count; j++) {
                        HR_TimeTask task = taskTimeLst[j];
                        if (task.tid == delTid) {
                            isDel = true;
            ...
            lock (lockFrame) {
                for (int i = 0; i < tmpDelFrameLst.Count; i++) {
                    ...
                    for (int j = 0; j < taskFrameLst.Count; j++) {
                        HR_FrameTask task = taskFrameLst[j];
                        if (task.tid == delTid) {
                            isDel = true;
                    ...
                    for (int j = 0; j < tmpFrameLst.Count; j++) {
                        HR_FrameTask task = tmpFrameLst[j];
                        if (task.tid == delTid) {
    ...
    private void CheckTimeTask() {
        ...
        for (int index = 0; index < taskTimeLst.Count; index++) {
            HR_TimeTask task = taskTimeLst[index];
    ...
    private void CheckFrameTask() {
        ...
        for (int index = 0; index < taskFrameLst.Count; index++) {
            HR_FrameTask task = taskFrameLst[index];
    ...
    #region TimeTask
    public int AddTimeTask(Action<int> callback, double delay, HR_TimeUnit timeUnit = HR_TimeUnit.Millisecond, int count = 1) {
        if (timeUnit != HR_TimeUnit.Millisecond) {
            switch (timeUnit) {
                case HR_TimeUnit.Second:
                    delay = delay * 1000;
                    break;
                case HR_TimeUnit.Minute:
                    delay = delay * 1000 * 60;
                    break;
                case HR_TimeUnit.Hour:
                    delay = delay * 1000 * 60 * 60;
                    break;
                case HR_TimeUnit.Day:
        ...
        nowTime = GetUTCMilliseconds();
        lock (lockTime) {
            tmpTimeLst.Add(new HR_TimeTask(tid, callback, nowTime + delay, delay, count));
        }
        return tid;
    }
    ...
    public bool ReplaceTimeTask(int tid, Action<int> callback, float delay, HR_TimeUnit timeUnit = HR_TimeUnit.Millisecond, int count = 1) {
        if (timeUnit != HR_TimeUnit.Millisecond) {
            switch (timeUnit) {
                case HR_TimeUnit.Second:
                    delay = delay * 1000;
        ...
        nowTime = GetUTCMilliseconds();
        HR_TimeTask newTask = new HR_TimeTask(tid, callback, nowTime + delay, delay, count);
    ...
    public int AddFrameTask(Action<int> callback, int delay, int count = 1) {
        int tid = GetTid();
        lock (lockTime) {
            tmpFrameLst.Add(new HR_FrameTask(tid, callback, frameCounter + delay, delay, count));
        }
        return tid;
    }
    ...
    public bool ReplaceFrameTask(int tid, Action<int> callback, int delay, int count = 1) {
        HR_FrameTask newTask = new HR_FrameTask(tid, callback, frameCounter + delay, delay, count);
    ...
    class HR_TimeTask {
        ...
        public HR_TimeTask(int tid, Action<int> callback, double destTime, double delay, int count) {
            this.tid = tid;
    ...
    class HR_FrameTask {
        ...
        public HR_FrameTask(int tid, Action<int> callback, int destFrame, int delay, int count) {
            this.tid = tid;
            this.callback = callback;
